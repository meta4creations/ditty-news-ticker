...
/*!*******************************************!*\
  !*** ./src/class-ditty-display-ticker.js ***!
  \*******************************************/
/* global jQuery:true */
...
(function ($) {
  ...
  Ditty_Ticker.prototype = {
    _init: function () {
      ...
      $contents = $('<div class="ditty__contents ditty-ticker__contents"></div>');
      this.$contents = $contents;

      // Create the ticker title
      this.$title = $('<div class="ditty__title ditty-ticker__title"></div>');
      this.$titleContents = $('<div class="ditty__title__contents ditty-ticker__title__contents"></div>');
      ...
      // Add listeners
      this.$elmt.on("mouseenter", {
        self: this
      }, this._mouseenter);
      this.$elmt.on("mouseleave", {
        self: this
      }, this._mouseleave);
    ...
    _timerStart: function () {
      ...
      function ditty_tickerLoop() {
        self.scrollIncrement = parseFloat(self.settings.speed) * self.scrollPercent;
        self._positionItems();
    ...
    _initializeItem: function (index, positionType) {
      ...
      if (this.settings.items[index].css) {
        dittyLayoutCss(this.settings.items[index].css, this.settings.items[index].layout_id, "update");
      }
    ...
    _fillTicker: function () {
      ...
      fillTimer = setInterval(function () {
        ...
        if ($item) {
          var data = self._filledItemInit(counter, $item, posX, posY, tickerW, tickerH);
          filled = data.filled;
    ...
    _styleItem: function ($item) {
      ...
      if ("" !== this.settings.itemElementsWrap) {
        $item.children(".ditty-item__elements").css({
          whiteSpace: this.settings.itemElementsWrap
        });
      }
      if ("" !== this.settings.itemMaxWidth) {
        $item.children(".ditty-item__elements").css({
          maxWidth: this.settings.itemMaxWidth
        });
      }
    },
    _renderPlayPauseButton: function () {
      if (this.settings.playPauseButton) {
        ...
        this.$playPauseButton.off("click", {
          self: this
        }, this._togglePlayPauseButton);
    ...
    destroy: function () {
      // Remove listeners
      this.$elmt.off("mouseenter", {
        self: this
      }, this._mouseenter);
      this.$elmt.off("mouseleave", {
        self: this
      }, this._mouseleave);
      this.$playPauseButton.off("click", {
        self: this
