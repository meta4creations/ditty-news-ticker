...
/*!*****************************************!*\
  !*** ./src/class-ditty-display-list.js ***!
  \*****************************************/
/* global jQuery:true */
...
(function ($) {
  ...
  Ditty_List.prototype = {
    ...
    _initSlider: function () {
      ...
      this.$contents.on("ditty_slider_init", {
        self: this
      }, this._triggerInit);
      this.$contents.on("ditty_slider_update", {
        self: this
      }, this._triggerUpdate);
      this.$contents.on("ditty_slider_after_slide_update", {
        self: this
    ...
    _destroySlider: function () {
      if (this.$contents.ditty_slider) {
        this.$contents.off("ditty_slider_init", {
          self: this
        }, this._triggerInit);
        this.$contents.off("ditty_slider_update", {
          self: this
        }, this._triggerUpdate);
        this.$contents.off("ditty_slider_after_slide_update", {
          self: this
        }, this._triggerShowSlide);
    ...
    _updateSlider: function (index) {
      var newIndex = index ? index : this.$contents.ditty_slider("options", "slide");
    ...
    _styleTitle: function () {
      ...
      const titleContentsPosition = this.settings.titleContentsPosition ? this.settings.titleContentsPosition : this.settings.titleElementPosition;
      const titleVerticalPosition = this.settings.titleElementVerticalPosition ? this.settings.titleElementVerticalPosition : this.settings.titleElementPosition;
      this.$elmt.attr("data-title_position", titleContentsPosition);
      this.$elmt.attr("data-title_horizontal_position", this.settings.titleElementPosition);
      ...
      if ("none" === this.settings.titleDisplay) {
        ...
        const titleElement = allowedTitleElements.includes(this.settings.titleElement) ? this.settings.titleElement : "h1"; // Default to 'h1' if the value is invalid

        var $element = $("<" + titleElement + ' class="ditty__title__element">' + this.settings.title + "</" + titleElement + ">");
    ...
    _calculatePages: function () {
      ...
      if (Boolean(this.settings.paging) && parseInt(this.settings.perPage) > 0) {
    ...
    _getPageByItemIndex: function (index) {
      var pageIndex = Math.ceil((parseInt(index) + 1) / parseInt(this.settings.perPage)) - 1;
      return pageIndex;
    },
    /**
    ...
    loadItems: function (newItems = [], swapType = "animate") {
      ...
      const {
        updatedItems,
        updatedIndexes
      } = dittyGetUpdatedItemData(this.settings.items, newItems);
      ...
      var $currentPage = this.$contents.ditty_slider("options", "currentSlide"),
        ...
        minIndex = Boolean(this.settings.paging) ? parseInt(this.settings.perPage) * pageIndex : 0,
        maxIndex = Boolean(this.settings.paging) ? parseInt(this.settings.perPage) * pageIndex + parseInt(this.settings.perPage) - 1 : currentItems.length + updatedItems.length,
        itemSwaps = [];
      if ($currentPage) {
        // Swap out items
      ...
      switch (key) {
        ...
        case "spacing":
          ...
          this.$elmt.find(".ditty-item").each(function () {
            $(this).css({
              paddingBottom: self.settings.spacing + "px"
            });
          });
          this.$elmt.find(".ditty-list__page").each(function () {
            $(this).children(".ditty-item:last-child").css({
              paddingBottom: 0
            });
          });
          break;
