...
(function ($) {
  ...
  Ditty_Ticker.prototype = {
    ...
    /**
     * Check if the ticker element is visible
     *
     * @since    3.0
     * @return   boolean
     */
    _checkVisibility: function () {
      ...
      while ($parent.length && !$parent.is("body, html")) {
        if (
          $parent.is(":hidden") ||
    ...
    _initVisibilityObserver: function () {
      ...
      if (
        ...
        !this.elmt ||
        !this.elmt.parentElement
      ) {
        // Fallback: check visibility periodically
        this._checkVisibilityFallback();
        return;
      }

      // Create observer with a small threshold to detect when element enters/exits viewport
      var observerOptions = {
      ...
      var observerCallback = function (entries) {
        entries.forEach(function (entry) {
          ...
          // Check both viewport intersection and parent visibility
          self.isVisible = entry.isIntersecting && self._checkVisibility();

          // Only handle state changes
          if (wasVisible !== self.isVisible) {
            if (self.isVisible) {
              // Element became visible - start if not paused
              if (!self.paused && !self.running) {
                self._timerStart();
              }
            } else {
              // Element became hidden - stop (but don't set paused)
              if (self.running) {
                self._timerStop();
              }
            }
      ...
      this.visibilityObserver.observe(this.elmt);

      // Also watch for style/attribute changes on parent elements
      if (typeof MutationObserver !== "undefined") {
        var mutationCallback = function (mutations) {
          var wasVisible = self.isVisible;
          self.isVisible = self._checkVisibility();
        ...
        // Observe all parent elements for style/attribute changes
        var $parent = this.$elmt.parent();
        while ($parent.length && !$parent.is("body, html")) {
          if ($parent[0] && $parent[0].nodeType === 1) {
            this.mutationObserver.observe($parent[0], {
              attributes: true,
      ...
      this.isVisible = this._checkVisibility();
    ...
    /**
     * Fallback visibility check using polling
     *
     * @since    3.0
     * @return   null
     */
    _checkVisibilityFallback: function () {
      var self = this;
      var checkInterval = setInterval(function () {
        if (!self.elmt || !self.elmt.parentElement) {
      ...
      this.visibilityCheckInterval = checkInterval;
    ...
    _initializeItems: function () {
      // Don't initialize if element is not visible
      if (!this.isVisible) {
        return;
      }
    ...
    _timerStart: function () {
      ...
      // Don't start if element is not visible
      if (!self.isVisible) {
        return false;
      }
      ...
      function ditty_tickerLoop() {
        ...
        self._positionItems();
        if (self.running && !self.paused && self.isVisible) {
    ...
    _fillTicker: function () {
      ...
      fillTimer = setInterval(function () {
        ...
        if (filled) {
          ...
          setTimeout(
            function () {
              if (!self.running && self.isVisible) {
                self._timerStart();
              }
            },
