...
export default class DittyTicker {
	...
	constructor(element, config = {}) {
		...
		// DOM references
		this.itemsContainer = element.querySelector('.ditty-display__items');
		this.originalItems = [];
		this.activeItems = [];
	...
	_init() {
		...
		const items = this.itemsContainer.querySelectorAll('.ditty-display__item');
		...
		// Mark as initialized
		this.element.classList.add('ditty-display--initialized');
	 ...
	 * Note: Padding is necessary for ticker animation with absolute positioning
	 * CSS gap doesn't work with absolutely positioned elements
	 */
	_applySpacing() {
	...
	_initializeItem(index) {
		...
		clone.classList.add('ditty-display__item--active');
	...
	_positionItems() {
		...
		for (let i = this.activeItems.length - 1; i >= 0; i--) {
			...
			if (this._shouldAddNewItem(item)) {
				if (
					this.config.cloneItems ||
					this.nextItemIndex < this.originalItems.length
				) {
	...
	_tick() {
		...
		this.scrollIncrement =
			parseFloat(this.config.speed) * this.config.scrollPercent;
	...
	destroy() {
		...
		this.activeItems.forEach(item => {
			if (
				item.element &&
				item.element.classList.contains('ditty-display__item--clone')
			) {
				item.element.remove();
			}
		});
		...
		// Reset state
		this.activeItems = [];
		this.element.classList.remove('ditty-display--initialized');
	}
}
