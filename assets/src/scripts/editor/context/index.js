...
export class EditorProvider extends Component {
	...
	initialDisplay = this.editorVars.displayObject
		...
		: false;
	initialSettings = (() => {
		const saved = this.editorVars.settings ? this.editorVars.settings : {};
		// Support dittyVersion with backwards compat for previewMode
		const dittyVersion = saved.dittyVersion || saved.previewMode || 'v4';
		return {
			status: 'publish',
			ajax_loading: 'no',
			live_updates: 'no',
			editorWidth: 350,
			dittyVersion,
			previewRenderMode: 'endpoint', // endpoint or srcdoc
			...saved,
			dittyVersion,
		};
	})();
	...
	state = {
		...
		currentPanel: 'items',
		previewVersion: 0, // Increment to force preview refresh
	};
	...
	handleDeleteDisplayItems = data => {
		const updates = Array.isArray(data) ? data : [data];
		const updatedDisplayItems = this.state.displayItems.filter(displayItem => {
			const itemIndex = updates.findIndex(
				update =>
	...
	handleUpdateTitle = updatedTitle => {
		// Update the Ditty options (only for v3 mode)
		const dittyVersion = this.state.settings.dittyVersion || this.state.settings.previewMode || 'v4';
		if (dittyVersion === 'v3') {
	...
	/**
	 * Refresh the preview by incrementing the version
	 * This forces the iframe to reload with new data
	 *
	 * @access public
	 * @since  4.0
	 */
	handleRefreshPreview = () => {
		this.setState(prevState => ({
			previewVersion: prevState.previewVersion + 1,
		}));

		// Log for debugging
		if (this.editorVars.mode === 'development' && window.console) {
			console.log('[Ditty Editor] Preview refreshed', {
				version: this.state.previewVersion + 1,
	...
	/**
	 * Set the Ditty version
	 *
	 * @param {string} mode - Ditty version: 'v3' | 'v4'
	 * @param {string} renderMode - For v4: 'endpoint' | 'srcdoc'
	 * @access public
	 * @since  4.0
	 */
	handleSetPreviewMode = (mode, renderMode = 'endpoint') => {
		const updatedSettings = { ...this.state.settings };
		updatedSettings.dittyVersion = mode;
		if (mode === 'v4') {
			updatedSettings.previewRenderMode = renderMode;
		}
		this.setState({ settings: updatedSettings });

		// Refresh preview to apply new version
			...
			<EditorContext.Provider
				value={{
					...
					actions: {
						...
						refreshPreview: this.handleRefreshPreview,
						setPreviewMode: this.handleSetPreviewMode,
					},
				}}
			>
				{this.props.children}
