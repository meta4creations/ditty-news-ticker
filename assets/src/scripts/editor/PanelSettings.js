...
const PanelSettings = () => {
	...
	const settingsFields = applyFilters('dittyEditor.settingsFields', [
		{
			...
			fields: [
				...
				},
				{
					type: 'radio',
					id: 'dittyVersion',
					name: __('Ditty Version', 'ditty-news-ticker'),
					help: __(
						'Choose which version to use for both the editor preview and frontend display.',
						'ditty-news-ticker'
					),
					options: {
						v3: __('V3 (Legacy)', 'ditty-news-ticker'),
						v4: __('V4 (New)', 'ditty-news-ticker'),
					},
					inline: true,
					std: 'v4',
				},
		...
		},
		{
			...
			fields: [
				{
					type: 'radio',
					id: 'previewRenderMode',
					name: __('V4 Rendering Method', 'ditty-news-ticker'),
					help: __(
						'Endpoint: Server-rendered with theme styles. Client-Side: Faster, generated in browser.',
						'ditty-news-ticker'
					),
					options: {
						endpoint: __('Server Endpoint', 'ditty-news-ticker'),
						srcdoc: __('Client-Side (srcdoc)', 'ditty-news-ticker'),
					},
					inline: true,
					std: 'endpoint',
					condition: {
	...
	const handleOnUpdate = (id, value) => {
		...
		} else if ('previewRenderMode' === id) {
			// When render mode changes, refresh preview
			const updatedSettings = _.cloneDeep(settings);
			updatedSettings[id] = value;
			actions.updateSettings(updatedSettings);
			actions.refreshPreview();
