	...
	public function enqueue_scripts( $hook ) {
		...
		if ( $ditty_id = ditty_editing() ) {
			...
			wp_enqueue_script( 'ditty-editor',
				...
				array_merge(['ditty-editor-init', 'wp-element', 'wp-components', 'wp-editor', 'wp-block-editor', 'wp-hooks', 'wp-tinymce', 'lodash', 'wp-codemirror', 'ditty'], $display_slugs),
				$this->version,
				true
			);
			if ( empty( $ditty_scripts_enqueued ) ) {
				...
				wp_add_inline_script( 'ditty-editor', 'const dittyEditorVars = ' . json_encode( array(
					...
					'id'								=> $ditty_id,
					'title' 						=> $title,
					'status'						=> ( 'ditty-new' == $ditty_id ) ? 'draft' : get_post_status( $ditty_id ),
		...
		if ( $display_id = ditty_display_editing() ) {
			...
			if ( empty( $ditty_scripts_enqueued ) ) {
				...
				wp_add_inline_script( 'ditty-display-editor', 'const dittyEditorVars = ' . json_encode( array(
					...
					'status'					=> ( 'ditty_display-new' == $display_id ) ? 'draft' : get_post_status( $display_id ),
