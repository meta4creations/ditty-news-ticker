...
// 		}
  ...
  /**
	...
	public function enqueue_scripts( $hook ) {
		...
		wp_register_script( 'ditty-slider', DITTY_URL . 'assets/build/dittySlider.js', array( 'jquery', 'hammer' ), $this->version, true );
		...
		if ( empty( $ditty_scripts_enqueued ) ) {
			wp_add_inline_script( 'ditty', 'const dittyVars = ' . wp_json_encode( apply_filters( 'dittyVars', array(
		...
		wp_register_script( 'ditty-editor-init',
			DITTY_URL . 'assets/build/dittyEditorInit.js',
      ...
      if ( $ditty_id = ditty_editing() ) {
        ...
        wp_add_inline_script( 'ditty-editor-init', 'const dittyEditorVars=' . wp_json_encode( apply_filters( 'dittyEditorVars', array(
          ...
          'googleFonts'          => ( 'enabled' == get_ditty_settings( 'disable_googlefonts' ) ),
          'dittyDevelopment'		 => defined( 'DITTY_DEVELOPMENT' ) ? DITTY_DEVELOPMENT : false
        ), $hook ), $json_flags ) . ';', 'before' );
      }
      
			wp_add_inline_script( 'ditty-editor-init', 'const dittyEditor=' . wp_json_encode( apply_filters( 'dittyEditor', array(
				'dittyDevelopment'	=> defined( 'DITTY_DEVELOPMENT' ) ? DITTY_DEVELOPMENT : false
			) ), $json_flags ) . ';', 'before' );
		}
      ...
      wp_enqueue_media();
			...
			if ( empty( $ditty_scripts_enqueued ) ) {
				...
				wp_add_inline_script( 'ditty-display-editor', 'const dittyEditorVars = ' . wp_json_encode( apply_filters( 'dittyEditorVars', array(
					'nonce'			          => wp_create_nonce( 'wp_rest' ),
					'mode'								=> WP_DEBUG ? 'development' : 'production',
					'userId'							=> get_current_user_id(),
					'restUrl'							=> get_rest_url(),
      ...
      wp_enqueue_media();
			...
			if ( empty( $ditty_scripts_enqueued ) ) {
				...
				wp_add_inline_script( 'ditty-layout-editor', 'const dittyEditorVars = ' . wp_json_encode( apply_filters( 'dittyEditorVars', array(
					'nonce'			      => wp_create_nonce( 'wp_rest' ),
					'mode'						=> WP_DEBUG ? 'development' : 'production',
					'userId'					=> get_current_user_id(),
					'restUrl'					=> get_rest_url(),
		...
		if ( is_admin() ) {
			...
			if ( ( 'ditty_page_ditty_export' == $hook || 'ditty_page_ditty_extensions' == $hook ) && current_user_can( 'manage_ditty_settings' ) ) {
				wp_enqueue_script( 'ditty-admin', DITTY_URL . 'assets/build/dittyAdmin.js', array(
					'jquery',
					'jquery-ui-core',
					'jquery-ui-sortable',
					'jquery-effects-core',
				...
				if ( empty( $ditty_scripts_enqueued ) ) {
					wp_add_inline_script( 'ditty-admin', 'const dittyAdminVars = ' . wp_json_encode( apply_filters( 'dittyAdminVars', array(
						'ajaxurl'				=> admin_url( 'admin-ajax.php' ),
						'security'			=> wp_create_nonce( 'ditty' ),
						'nonce'			    => wp_create_nonce( 'wp_rest' ),
						'mode'					=> WP_DEBUG ? 'development' : 'production',
			...
			if ( 'ditty_page_ditty_settings' == $hook && current_user_can( 'manage_ditty_settings' ) ) {
				wp_enqueue_script( 'ditty-settings',
					DITTY_URL . 'assets/build/dittySettings.js',
					['wp-element', 'wp-components', 'wp-hooks', 'wp-sanitize', 'lodash', 'ditty-editor-init'],
					$this->version,
					true
				);
				wp_add_inline_script( 'ditty-settings', 'const dittySettingsVars = ' . wp_json_encode( apply_filters( 'dittySettingsVars', array(
					'ajaxurl'						=> admin_url( 'admin-ajax.php' ),
          'nonce'             => wp_create_nonce('wp_rest'),
					'userId'						=> get_current_user_id(),
					'restUrl'						=> get_rest_url(),
