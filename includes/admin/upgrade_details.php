...
function ditty_info_page() {
	add_submenu_page(
		...
		__( 'Ditty Upgrade Details', 'ditty-news-ticker' ),		// The value used to populate the browser's title bar when the menu page is active
		...
		'upgrade_details',						// The ID used to represent this submenu item
		'upgrade_details_display'			// The callback function used to render the options for this submenu item
	);
}
add_action( 'admin_menu', 'ditty_info_page', 9 );

function upgrade_details_display() {
	?>
	<div id="ditty-page" class="wrap">
		...
		<div id="ditty-page__header">
			...
			<h1 id="ditty-page__logo"><img id="ditty-page__logo__image" src="<?php echo DITTY_URL; ?>includes/img/ditty-logo-black.png" alt="<?php _e( 'Ditty', 'ditty-news-ticker' ); ?>" /></h1>
			<h3><?php _e( 'News Ticker was just the beginning' ); ?></h3>
		...
		<div id="ditty-page__content">
			<section class="ditty-section ditty-section--intro">
				...
				<p style="font-size:16px;margin-bottom:0;"><?php _e( "Don't worry, all of your existing <strong>News Tickers</strong> will still work! Although, we do urge you to start upgrading and updating your tickers to the new <strong>Ditty</strong> post type. <strong>Ditty News Ticker</strong> is now relagated to legacy code and there will be very limited updates from this point on. Most development time will now be assigned to <strong>Ditty</strong> and existing and new <strong>Ditty</strong> extensions.", 'ditty-news-ticker' ); ?></p>
			</section>
