...
(function ($) {
  ...
  Ditty_Items_Panel.prototype = {
    ...
    _init: function () {
      ...
      // Add actions
      this.settings.editor.$elmt.on(
        "ditty_editor_active_items_update",
        { self: this },
        this._dittyActiveItemsUpdated
      );
      ...
      this.$elmt.on(
        "click",
        ".ditty-data-list__item__edit",
        { self: this },
        this._editItem
      );
      this.$elmt.on(
        "click",
        ".ditty-data-list__item__layout",
        { self: this },
        this._editLayoutVariations
      );
      this.$elmt.on(
        "click",
        ".ditty-data-list__item__clone",
        { self: this },
        this._cloneItem
      );
      this.$elmt.on(
        "click",
        ".ditty-data-list__item__delete",
        { self: this },
        this._deleteItem
      );
      this.$list.on(
      ...
      $("body").on(
        "ditty_editor_save_ditty_response",
        { self: this },
        this._dittyEditorSaveResponse
      );
    ...
    dittyUpdateSavedDraftLayouts: function (draftId, newID) {
      $.each($(".ditty-editor-item"), function () {
        ...
        $(this)
          .attr("data-layout_value", layoutValue)
          .data("layout_value", layoutValue);
      });
    },
    ...
    _initializeSorting: function () {
      ...
      this.$listItems.sortable({
        ...
        stop: function (event, ui) {
          ...
          self.settings.editor.addUpdate(
            "item_order",
            self.settings.editor.dittyId
    ...
    _showItem: function (e) {
      ...
      if (!$(e.target).is("a") && !$(e.target).parent().is("a")) {
        var $item = $(e.target).is(".ditty-data-list__item")
            ? $(e.target)
            : $(e.target).parents(".ditty-data-list__item"),
    ...
    _actionClick: function (e) {
      ...
      $("body").trigger("ditty_editor_item_action_click", [
        $button,
        $item,
        itemId,
        dittyId,
    ...
    _editItem: function (e) {
      ...
      $.post(dittyVars.ajaxurl, data, function (response) {
        if (response) {
          var html =
            '<div class="ditty-editor__panel ditty-editor__panel--item_editor">' +
            response +
            "</div>";
    ...
    _editLayoutVariations: function (e) {
      ...
      $.post(dittyVars.ajaxurl, data, function (response) {
        if (response) {
          var html =
            '<div class="ditty-editor__panel ditty-editor__panel--layout-variations">' +
            response +
            "</div>";
          self.settings.editor.panelOptions("transition", "slideLeft");
          ...
          if (response.display_items) {
            if (self.isEmptyTicker) {
              var placeholderItems =
                self.settings.editor.ditty.options("items");
      ...
      $.post(
        ...
        function (response) {
          if (response.display_items && response.display_items.length) {
            self.settings.editor.ditty.updateItems(
              response.display_items,
              itemId,
              "after"
            );
          }
    ...
    _updateItemIndexes: function (action) {
      ...
      if ("updateDitty" === action) {
        ...
        var reorderedItems = dittyItemsReorder(
          self.settings.editor.ditty.options("items"),
          itemIds
        );
